...
async function testWebhook() {
  ...
  `;
    const subitemResult = await mondayQuery(subitemQuery, {
        itemId: [SUBITEM_ID],
    });
    ...
    let linkedMainItemIds = [];
    if (relationColumn?.value) {
        try {
            const parsed = JSON.parse(relationColumn.value);
            linkedMainItemIds = parsed.linkedPulseIds?.map((p) => p.linkedPulseId) || [];
        }
        catch {
    ...
    `;
        const mainItemResult = await mondayQuery(mainItemQuery, {
        ...
        if (mainRelationColumn?.value) {
            try {
                ...
                existingLinkedIds = parsed.linkedPulseIds?.map((p) => p.linkedPulseId) || [];
            }
            catch {
